...
async def _poll_redis(key: str, deadline: float) -> str:
    """Async Redis poll — yields control to the event loop between checks."""
...
def await_approval(state: IncidentState) -> dict:
    ...
    try:
        ...
        decision = loop.run_until_complete(_poll_redis(key, deadline))
    except RuntimeError:
        # already inside a running loop (e.g. tests) — fall back to sync poll
        decision = "timeout"
        while time.time() < deadline:
            val = _r.get(key)
            if val in ("approved", "rejected"):
    ...
    return {
        "approval": decision,
        "decision_trail": [f"hitl: human {decision} after {elapsed}s"],
    }
